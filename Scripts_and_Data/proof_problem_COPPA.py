...
#this script allows to generate problem for Slackje to prove if a certain website requires to comply to COPPA
import re
import sys
import os

#rules: the rules are already present in the axioms.ax file
axiom_file = 'axioms_coppa.ax'
...
#we get the data from the script formal_model_extraction.py that generates different files among which the include and redirect relationships and the blocked domains by the extensions. This is all we need, therefore we do not need
#to wait until formal_model_extraction.py is ready: just use dom_blocked.txt and edges_tracking_flow.csv 
def main():
	global complete_match
	if len(sys.argv)<2:
		print('Usage: python proof_problem_generation.py domain_require_coppa')
		exit(1)
	...
	#add kids domains
	add_kids(domains)
	#explicitly says that domains are different e.g. 'facebook.com'!='google.com': IT GENERATES AN HUGE FILE SO DO NOT IMPLEMENT IT FOR THE MOMENT
	#add_domains_different(domains)
...
def add_kids(domains):
	with open(kids_domains,'r') as file_kids, open(tmp_problem_input,'a') as f:
		lines = file_kids.readlines()
		for kid_website in lines:
			kid_website = kid_website.strip('\n')
			if kid_website in domains:
				string = "fof(%s,axiom,(kids(\"%s\"))).\n" % (var, kid_website)
				f.write(string)
				#plus we force to visit to trigger any knows from external website that will generate COPPA compliance
				string_visit = "fof(%s,axiom,(visit(\"%s\"))).\n" % (var, kid_website)
				f.write(string_visit)

def add_domains_different(domains):
...
#it is also possible to ask for a generic match i.e. is there a website that must comply to COPPA?
#in this case the formula is: fof(%s,conjecture,(? [W] : (req_coppa(W))))\n. % (var)
def add_goal(domain1):
	with open(tmp_problem_input,'a') as f:
		string = "fof(%s,conjecture,(req_coppa(\"%s\"))).\n" % (var,domain1)
		f.write(string)
